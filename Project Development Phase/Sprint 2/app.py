...
try:
    ...
    conn_str='database=bludb;hostname=3883e7e4-18f5-4afe-be8c-fa31c41761d2.bs2io90l08kqb1od8lcg.databases.appdomain.cloud;port=31498;protocol=tcpip;uid=ghg49611;pwd=LinZpDEpVE4iVw2W;security=SSL'
...
def addexpense():
    ...
    param = "SELECT * FROM expenses WHERE userid = " + str(userid) + " AND MONTH(date) = MONTH(current timestamp) AND YEAR(date) = YEAR(current timestamp) ORDER BY date DESC"
    res = ibm_db.exec_immediate(ibm_db_conn, param)
    dictionary = ibm_db.fetch_assoc(res)
    expense = []
    while dictionary != False:
    ...
    print("total : ",total)
    param = "SELECT userid, limitss FROM limits WHERE userid = " + str(userid) + " ORDER BY userid DESC LIMIT 1"
    res = ibm_db.exec_immediate(ibm_db_conn, param)
    dictionary = ibm_db.fetch_assoc(res)
    row = []
    s = 0
    ...
    if total > int(s):
        ...
        #sendmail.Sendmail(msg,session['email'])  
...
def display():
    ...
    param = "SELECT * FROM expenses WHERE userid = " + str(session['userid']) + " ORDER BY date DESC"
    res = ibm_db.exec_immediate(ibm_db_conn, param)
    dictionary = ibm_db.fetch_assoc(res)
    expense = []
    while dictionary != False:
...
@app.route('/update/<userid>', methods = ['POST','GET'])
def update(userid):
  if request.method == 'POST' :
    ...
    #   mysql.connection.commit()
      ...
      stmt = ibm_db.prepare(ibm_db_conn, sql)
      ibm_db.bind_param(stmt, 1, p1)
      ibm_db.bind_param(stmt, 2, expensename)
      ibm_db.bind_param(stmt, 3, amount)
      ibm_db.bind_param(stmt, 4, paymode)
      ibm_db.bind_param(stmt, 5, category)
      ibm_db.bind_param(stmt, 6, userid)
      ibm_db.execute(stmt)
...
def limitn():
    ...
    param = "SELECT userid, limitss FROM limits WHERE userid = " + str(session['userid']) + " ORDER BY userid DESC LIMIT 1"
    res = ibm_db.exec_immediate(ibm_db_conn, param)
    dictionary = ibm_db.fetch_assoc(res)
    row = []
    s = " /-"
...
def today():
    ...
    #   print(texpense)
      ...
      param1 = "SELECT TIME(date) as tn, amount FROM expenses WHERE userid = " + str(session['userid']) + " AND DATE(date) = DATE(current timestamp) ORDER BY date DESC"
    ...
    #   expense = cursor.fetchall()
      ...
      param = "SELECT * FROM expenses WHERE userid = " + str(session['userid']) + " AND DATE(date) = DATE(current timestamp) ORDER BY date DESC"
      res = ibm_db.exec_immediate(ibm_db_conn, param)
      dictionary = ibm_db.fetch_assoc(res)
      expense = []
      while dictionary != False:
...
def month():
    ...
    #   print(texpense)
      ...
      param1 = "SELECT DATE(date) as dt, SUM(amount) as tot FROM expenses WHERE userid = " + str(session['userid']) + " AND MONTH(date) = MONTH(current timestamp) AND YEAR(date) = YEAR(current timestamp) GROUP BY DATE(date) ORDER BY DATE(date)"
      res1 = ibm_db.exec_immediate(ibm_db_conn, param1)
      dictionary1 = ibm_db.fetch_assoc(res1)
      texpense = []
    ...
    #   expense = cursor.fetchall()
      ...
      param = "SELECT * FROM expenses WHERE userid = " + str(session['userid']) + " AND MONTH(date) = MONTH(current timestamp) AND YEAR(date) = YEAR(current timestamp) ORDER BY date DESC"
      res = ibm_db.exec_immediate(ibm_db_conn, param)
      dictionary = ibm_db.fetch_assoc(res)
      expense = []
      while dictionary != False:
...
def year():
    ...
    #   print(texpense)
      ...
      param1 = "SELECT MONTH(date) as mn, SUM(amount) as tot FROM expenses WHERE userid = " + str(session['userid']) + " AND YEAR(date) = YEAR(current timestamp) GROUP BY MONTH(date) ORDER BY MONTH(date)"
      res1 = ibm_db.exec_immediate(ibm_db_conn, param1)
      dictionary1 = ibm_db.fetch_assoc(res1)
      texpense = []
    ...
    #   expense = cursor.fetchall()
      ...
      param = "SELECT * FROM expenses WHERE userid = " + str(session['userid']) + " AND YEAR(date) = YEAR(current timestamp) ORDER BY date DESC"
      res = ibm_db.exec_immediate(ibm_db_conn, param)
      dictionary = ibm_db.fetch_assoc(res)
      expense = []
      while dictionary != False:
